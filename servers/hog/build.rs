...
fn main() {
    ...
    // 0xFFC2000000 = its 1 TiB RAM base minus virt's 1 GiB base, plus a
    // 32 MiB offset so the server map clears the sbsa kernel image).
    ...
    println!("cargo:rustc-link-arg=--defsym=LINK_BASE=0x{:x}", 0x410E0000u64 + shift);
}
