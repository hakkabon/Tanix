...
harness, docs, syscall-coverage pass) is now underway**: a GitHub
Actions workflow (`.github/workflows/ci.yml`) lints and boots both
machine targets on every push, `docs/SYSCALLS.md` tracks syscall
coverage, and `CHANGELOG.md` reconstructs the phase history — see
[Continuous integration](#continuous-integration) and
[Known limitations](#known-limitations) for what's still open.
...
| 22 | QEMU CI harness (`virt` + `sbsa-ref`, SMP + net regression boots), docs pass, syscall coverage | 🚧 in progress |
...
- **`lint`** — `just check` + `just lint-all` (clippy, whole workspace,
  warnings as errors) on the pinned `nightly-2026-07-01` toolchain.
- **`virt-phase11-boot`** — builds `kernel-phase11` and boots it under
  QEMU, asserting on serial-console markers that the Minix-style server
  set *and* the socket stack actually came up (not just that the image
  built).
- **`virt-smp-boot`** — boots the same image with `-smp 4` and asserts
  all three secondary CPUs report in via PSCI `CPU_ON`.
- **`virt-net-e2e`** — runs the existing `scripts/net-test.sh` end to
  end (UDP in/out, TCP echo, TCP out) against the real wire paths.
- **`sbsa-boot`** — builds `kernel-sbsa` and asserts both the EL3
  monitor's secure-world console and the Phase 20 filesystem server came
  up.
- **`sbsa-rtos-boot`** — builds `kernel-sbsa-rtos` and asserts the Phase
  21 co-tenant RTOS guest completes its demo.

The boot-assertion logic lives in `scripts/boot-test.sh`, a small
generic "boot in QEMU, wait for console markers, fail loud on timeout"
tool usable locally as well as in CI — see the script's header for
usage. A green CI run is a meaningfully stronger signal than "it
compiled": every job boots the real image and checks it did something,
not just that `cargo build` exited 0.

`docs/SYSCALLS.md` tracks which server crates exercise each of the 27
syscalls (26 have at least one real caller today); regenerate the raw
data with `just syscall-coverage`. `CHANGELOG.md` reconstructs the phase
history from `git log` for anyone who wants the "what shipped when"
view instead of the roadmap table above.

## Known limitations

- **Phase 22 is in progress, not finished.** CI now boots both machine
  targets and asserts real console markers, and syscall coverage /
  changelog docs exist — but the coverage table is regenerated by hand
  (`just syscall-coverage` doesn't yet fail CI on drift), and the CI
  boot jobs cover the `phase11`/`sbsa`/`sbsa-rtos` milestones only, not
